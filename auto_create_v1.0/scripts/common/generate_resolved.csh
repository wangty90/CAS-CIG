...
if ($COMP_WRF != 'none') then
./$COMP_WRF.*template || echo "ERROR: generate_resolved.csh error for wrf template"  && exit -1
endif
if ($COMP_GEA != 'none') then
./$COMP_GEA.*template || echo "ERROR: generate_resolved.csh error for gea template"  && exit -1
endif
