...
!<list> key="iowrite" number=1
         if ({ccc}_present) then
            call seq_cdata_setptrs(cdata_{c}x,gsmap=gsmap)
            call seq_io_write(rest_file,gsmap,fractions_{c}x,'fractions_{c}x',whead=whead,wdata=wdata)
            call seq_io_write(rest_file,gsmap,{c}2x_{c}x,'{c}2x_{c}x',whead=whead,wdata=wdata)
...
!<list> key="iowrite" number=4
         if ({ccc}_present) then
            call seq_cdata_setptrs(cdata_{c}x,gsmap=gsmap)
            call seq_io_write(rest_file,gsmap,fractions_{c}x,'fractions_{c}x',whead=whead,wdata=wdata)
            call seq_io_write(rest_file,gsmap,{c}2x_{c}x,'{c}2x_{c}x',whead=whead,wdata=wdata)
            call seq_io_write(rest_file,gsmap,x2{c}acc_{c}x%data,'x2{c}acc_{c}x',whead=whead,wdata=wdata)
            call seq_io_write(rest_file,      x2{c}acc_{c}x_cnt,'x2{c}acc_{c}x_cnt',whead=whead,wdata=wdata)
            call seq_io_write(rest_file,gsmap,xa{c}_{c}x,'xa{c}_{c}x',whead=whead,wdata=wdata)
            call seq_cdata_setptrs(cdata_ax,gsmap=gsmap)
            call seq_io_write(rest_file,gsmap,xa{c}_ax,'xa{c}_ax',whead=whead,wdata=wdata)
         endif
!</list>

!<list> key="iowrite" number=3
         if ({ccc}_present .and. ocn{ccc}_prognostic) then
            call seq_cdata_setptrs(cdata_{c}x,gsmap=gsmap)
            call seq_io_write(rest_file,gsmap,{c}2xacc_{c}x%data,'{c}2xacc_{c}x',whead=whead,wdata=wdata)
            call seq_io_write(rest_file,      {c}2xacc_{c}x_cnt,'{c}2xacc_{c}x_cnt',whead=whead,wdata=wdata)
         endif
!</list>


!<list> key="iowrite" number=6
         if ({ccc}_present) then
            call seq_cdata_setptrs(cdata_{c}x,gsmap=gsmap)
            call seq_io_write(rest_file,gsmap,fractions_{c}x,'fractions_{c}x',whead=whead,wdata=wdata)
         endif
...
!<list> key="iowrite" number=5
         if ({ccc}_present) then
            call seq_cdata_setptrs(cdata_{c}x,gsmap=gsmap)
            call seq_io_write(rest_file,gsmap,x2{c}_{c}x,'x2{c}_{c}x',whead=whead,wdata=wdata)
         endif
      enddo
!</list>
