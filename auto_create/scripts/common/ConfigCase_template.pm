...
# {list} key="ocnicecomp"
#   if ($self->get("COMP_{comp1}") eq "{comp2}") {if ($filename =~ "env_build") { if (($format eq "xml") || ($filename =~ m/xml/)) {  if ($xmode =~ "expert") {self->_write_xml2($fh, "build_{comp2}");} else { $self->_write_xml($fh, "build_{comp2}");  print $fh "\n<!-- ====================================== -->"; } } else {$self->_write_env($fh, "build_{comp2}"); }}if ($filename =~ "env_mach_pes") {   if (($format eq "xml") || ($filename =~ m/xml/)) {$self->_write_xml2($fh, "mach_pes_{comp2}"); } else {$self->_write_env($fh, "mach_pes_{comp2}");}}}
