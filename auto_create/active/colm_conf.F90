...
[end 5]
[start 6]
    type(seq_cdata),            intent(inout) :: cdata_r          ! Inpu runoff-model driver data
...
[start 10]
    ...
    ! If downscale, map this from the clm internal grid (dst) to the clm driver
    ! grid (src)
...
!      call seq_infodata_GetData(infodata, glc_g2supdate =
!      update_glc2sno_fields)
...
!   endif ! create_glacier_mec_landunit
    ...
    ! Finish accumulation of attribute vector and average and zero out partial
    ! sum and counter
...
!---------------------------------------------------------------------------
    ...
    do g = begg,endg
        ...
        ! Note that the following does unit conversions from ppmv to partial
        ! pressures (Pa)
...
!---------------------------------------------------------------------------
    ...
    do g = begg,endg
       ...
       ! These are now standard fields, but the check on the index makes sure
       ! the driver handles them
...
!---------------------------------------------------------------------------
    ...
    ! Note that in addition land carries around landfrac for the purposes of
    ! domain checking
    !
    ...
    allocate(data(lsize))
    !
    ! Determine global gridpoint number attribute, GlobGridNum, which is set
    ! automatically by MCT
    !
    call mct_gsMap_orderedPoints(gsMap_r, iam, idata)
    call mct_gGrid_importIAttr(dom_r,'GlobGridNum',idata,lsize)
    !
    ! Determine domain (numbering scheme is: West to East and South to North to
    ! South pole)
...
!---------------------------------------------------------------------------
    ...
    do n = beg_ocn_rof, end_ocn_rof
       ni = ni + 1
     ! liquid and ice runoff are bundled together to liquid runoff, and then ice
     ! runoff set to zero
